...
public class QuadTree<T> {
    ...
    private double MAX_OBJ_TARGET_EXPONENT = 0.333333;
    private int size = 0;
    private ListProvider<CoordHolder> LIST_PROVIDER = ListProvider.LP_LINKEDLIST;
    ...
    public class CoordHolder {
        ...
        private CoordHolder(double x, double y, T o, Quad quad) {
            this.x = x;
            this.y = y;
            this.data = o;
            this.quad = quad;
        ...
        private void remove() {
            if (quad.items.remove(this)) size--;
        }
    ...
    private class Quad {
        private Quad parent = null, UL = null, // upper left corner child ...
                UR = null, LL = null, LR = null;
        private double x1, y1, x2, y2;
        private List<CoordHolder> items = LIST_PROVIDER.getList(LEAF_MAX_OBJECTS, size);


        private Quad(Quad parent, double x1, double y1, double x2, double y2) {
            this.parent = parent;
            this.x1 = x1;
            this.y1 = y1;
            this.x2 = x2;
        ...
        private void findAll(double X1, double Y1, double X2, double Y2, List<CoordHolder> ret) {
            if (UL == null) {
                for (CoordHolder h : items)
                    if (h.x >= X1 && h.y >= Y1 && h.x <= X2 && h.y <= Y2) ret.add(h);
                return;
            }
        ...
        private CoordHolder place(double x, double y, T o) {
            return place(new CoordHolder(x, y, o, null), 0);
        }

        private CoordHolder place(CoordHolder h, int n) {
            double x = h.x, y = h.y;
        ...
        private Quad initParent(double x, double y) {
            int quadInd = 0;
            double X1 = x1, Y1 = y1, X2 = x2 + (x2 - x1), Y2 = y2 + (y2 - y1);
            if (x < X1) {
                quadInd++;
                X1 -= (x2 - x1);
                X2 -= (x2 - x1);
        ...
        private void replace(CoordHolder item, int n) {
        ...
        private void print(PrintStream out, int indent) {
            ...
            for (CoordHolder h : this.items) {
                ...
                out.print(h.y);
                out.print(": ");
                out.println(h.data.toString().replaceAll("\r?\n.*", ""));
            }
            if (UL != null) {
    ...
    private void adjustMaxObjects() {
        this.LEAF_MAX_OBJECTS = Math.max(7, (int) Math.pow(size, MAX_OBJ_TARGET_EXPONENT));
    }
