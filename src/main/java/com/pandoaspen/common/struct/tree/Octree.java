...
public class Octree<T> {
    ...
    public class Oct {
        public Oct parent = null, ULN = null, // upper left corner near child ...
                ULF = null, // u,l, far
                URN = null, URF = null, LLN = null, LLF = null, LRN = null, LRF = null;
        ...
        public void findAll(double X1, double Y1, double Z1, double X2, double Y2, double Z2, List<CoordHolder> ret) {
            if (ULN == null) {
                for (CoordHolder h : items)
                    if (h.x >= X1 && h.y >= Y1 && h.z >= Z1 && h.x <= X2 && h.y <= Y2 && h.z <= Z2) ret.add(h);
                return;
            }
        ...
        private CoordHolder place_(CoordHolder h, int n) {
            ...
            if (x < x1 || y < y1 || z < z1 || x > x2 || y > y2 || z > z2) {
                if (x1 == x2) {
                    ...
                    y1 = Math.min(y1, y);
                    z1 = Math.min(z1, z);
                    double add = Math.max(Math.max(Math.max(x2, x) - x1, Math.max(y2, y) - y1), Math.max(z2, z) - z2);
                    x2 = x1 + add;
                    y2 = y1 + add;
                    z2 = z1 + add;
                } else {
        ...
        private Oct initParent(double x, double y, double z) {
            int octInd = 0;
            double X1 = x1, Y1 = y1, Z1 = z1, X2 = x2 + (x2 - x1), Y2 = y2 + (y2 - y1), Z2 = z2 + (z2 - z1);
            if (x < X1) {
                octInd++;
                X1 -= (x2 - x1);
                X2 -= (x2 - x1);
    ...
    public List<CoordHolder> findAll(double x, double y, double z, double radius) {
        List<Octree<T>.CoordHolder> list =
                findAll(x - radius, y - radius, z - radius, x + radius, y + radius, z + radius);
        ...
        if (list instanceof LinkedList) {
            ...
            while (it.hasNext()) {
                ...
                double dist = Math.pow(holder.x - x, 2) + Math.pow(holder.y - y, 2) + Math.pow(holder.z - z, 2);
                if (dist > r2) {
                    it.remove();
                }
            }
