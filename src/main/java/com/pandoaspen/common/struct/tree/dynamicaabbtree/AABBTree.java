...
public final class AABBTree<T extends Boundable & Identifiable> {
    ...
    public static final float DEFAULT_FAT_AABB_MARGIN = 1 / 16f;
    ...
    private transient final AABBTreeHeuristicFunction<T> insertionHeuristicFunction = new AreaAABBHeuristicFunction<>();
    private transient final AABBOverlapFilter<T> defaultAABBOverlapFilter = new DefaultAABBOverlapFilter<>();
    private transient final CollisionFilter<T> defaultCollisionFilter = new DefaultCollisionFilter<>();
    private transient final FrustumIntersection frustumIntersection = new FrustumIntersection();
    private transient final RayAabIntersection rayIntersection = new RayAabIntersection();

    private int root;
    private float fatAABBMargin = DEFAULT_FAT_AABB_MARGIN;
    ...
    private AABBTreeNode<T> balanceNode(AABBTreeNode<T> node) {
        ...
        if (balance > 1) {
            if (right.getLeftChild() != -1 && right.getRightChild() != -1) {
                return balanceRight(node, left, right);
            }
        }
        if (balance < -1) {
            if (left.getLeftChild() != -1 && left.getRightChild() != -1) {
    ...
    private void syncUpHierarchy(AABBTreeNode<T> startingPoint) {
        AABBTreeNode<T> node = startingPoint.getParent() == AABBTreeNode.INVALID_NODE_INDEX ? null :
                getNodeAt(startingPoint.getParent());
    ...
    private void insertNode(int node) {
        ...
        while (!parentNode.isLeaf()) {
            ...
            AABBTreeHeuristicFunction.HeuristicResult heuristicResult =
                    insertionHeuristicFunction.getInsertionHeuristic(aabbA, aabbB, nodeToAdd.getData(), nodeToAddAABB);
            parentNode =
                    AABBTreeHeuristicFunction.HeuristicResult.LEFT.equals(heuristicResult) ? leftChild : rightChild;
        }
    ...
    private void detectCollisionPairsWithNode(AABBTreeNode<T> nodeToTest, CollisionFilter<T> filter,
                                              Set<CollisionPair<T>> alreadyTested, Set<CollisionPair<T>> result) {
    ...
    public void remove(T object) {
        ...
        AABBTreeNode<T> nodeParent =
                node.getParent() == AABBTreeNode.INVALID_NODE_INDEX ? null : getNodeAt(node.getParent());
        AABBTreeNode<T> nodeGrandparent =
                nodeParent == null || nodeParent.getParent() == AABBTreeNode.INVALID_NODE_INDEX ? null :
                        getNodeAt(nodeParent.getParent());
        AABBTreeNode<T> nodeSibling = null;
        if (nodeParent != null) {
            nodeSibling = nodeParent.getLeftChild() == objectNodeIndex ? getNodeAt(nodeParent.getRightChild()) :
                    getNodeAt(nodeParent.getLeftChild());
    ...
    public void detectOverlaps(AABBf overlapWith, Set<T> result) {
        detectOverlaps(overlapWith, defaultAABBOverlapFilter, result);
    }

    public void detectOverlaps(AABBf overlapWith, AABBOverlapFilter<T> filter, Set<T> result) {
        traverseTree(aabb -> aabb.testAABB(overlapWith), filter, result);
    }

    public void detectCollisionPairs(Set<CollisionPair<T>> result) {
        detectCollisionPairs(defaultCollisionFilter, result);
    }

    public void detectCollisionPairs(CollisionFilter<T> filter, Set<CollisionPair<T>> result) {
        result.clear();
        if (root == AABBTreeNode.INVALID_NODE_INDEX) {
            return;
        }
    ...
    public void detectInFrustum(Matrix4fc worldViewProjection, Set<T> result) {
        detectInFrustum(worldViewProjection, defaultAABBOverlapFilter, result);
    }

    public void detectInFrustum(Matrix4fc worldViewProjection, AABBOverlapFilter<T> filter, Set<T> result) {
        frustumIntersection.set(worldViewProjection, false);
        traverseTree(
                aabb -> frustumIntersection.testAab(aabb.minX, aabb.minY, aabb.minZ, aabb.maxX, aabb.maxY, aabb.maxZ),
                filter, result);
    }

    public void detectRayIntersection(Rayf ray, Set<T> result) {
        detectRayIntersection(ray, defaultAABBOverlapFilter, result);
    }

    public void detectRayIntersection(Rayf ray, AABBOverlapFilter<T> filter, Set<T> result) {
        rayIntersection.set(ray.oX, ray.oY, ray.oZ, ray.dX, ray.dY, ray.dZ);
        traverseTree(aabb -> rayIntersection.test(aabb.minX, aabb.minY, aabb.minZ, aabb.maxX, aabb.maxY, aabb.maxZ),
                filter, result);
    }

    private void traverseTree(Predicate<AABBf> nodeTest, AABBOverlapFilter<T> filter, Set<T> result) {
        result.clear();
        if (root == AABBTreeNode.INVALID_NODE_INDEX) {
            return;
        }
    ...
    /**
     * Note: Only applicable to minecraft world size, as height is capped at 256
     *
     * @return
     */
    ...
    public Stream<T> streamAll() {
        return objects.keySet().stream().map(AABBTreeObject::getData);
    }
